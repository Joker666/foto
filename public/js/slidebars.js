...
;(function($) {
	...
	$.slidebars = function(options) {
		...
		if (android && android < 3 || iOS && iOS < 5) $('body').addClass('sb-static'); // Add helper class for older versions of Android & iOS.
		...
		function initialise() {
			if (!settings.disableOver || (typeof settings.disableOver === 'number' && settings.disableOver >= windowWidth)) { // False or larger than window size. 
				...
				$('body').addClass('sb-init'); // Add helper class.
				if (settings.hideControlClasses) $controls.removeClass('sb-hide'); // Remove class just incase Slidebars was originally disabled.
			} else if (typeof settings.disableOver === 'number' && settings.disableOver < windowWidth) { // Less than window size.
				init = false; // false stop Slidebars from opening.
				$('body').removeClass('sb-init'); // Remove helper class.
		...
		function open(side) {
			...
			function proceed() {
				if (init && side === 'left' && $left) { // Slidebars is initiated, left is in use and called to open.
					$('body').addClass('sb-active sb-active-left'); // Add active classes.
					$left.addClass('sb-active');
					animate($left, $left.css('width'), 'left'); // Animation
					setTimeout(function() { leftActive = true; }, 400); // Set active variables.
				} else if (init && side === 'right' && $right) { // Slidebars is initiated, right is in use and called to open.
					$('body').addClass('sb-active sb-active-right'); // Add active classes.
		...
		function close(link) {
			if (leftActive || rightActive) { // If a Slidebar is open.
				...
				setTimeout(function() { // Wait for closing animation to finish.
					$('body').removeClass('sb-active sb-active-left sb-active-right'); // Remove active classes.
